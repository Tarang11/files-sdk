...
### `url()` throws on two configurations

`url()` returns the most direct URL each adapter can produce - a signed `GetObject`, a SAS read URL, a CDN URL when `publicBaseUrl` is configured, etc. Two setups have no URL primitive and will throw:
